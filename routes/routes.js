...
router.post('/login', passport.authenticate('local', {failureRedirect: '/'}), controller.handle_login);

router.get('/register', controller.handle_registration);

router.get('/profile', ensureAuthenticated, controller.load_profile);

router.get('/profile/switch/:id', ensureAuthenticated, controller.toggle_completed );

router.get('/profile/completed', ensureAuthenticated, controller.load_completed);
